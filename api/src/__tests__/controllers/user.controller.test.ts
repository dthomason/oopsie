...
describe('POST api/user', () => {
  describe('Creating an account', () => {
    it.only('returns 200, token and Info', async () => {
      ...
      //       expect(returned.mobile).toBe(newUser.mobile);
      ...
      //
      //       expect(payload?.mobile).toBe(newUser.mobile);
...
describe('POST /api/user/signin', () => {
  ...
  describe('incorrect creds', () => {
    ...
    it('returns 400 with message', async () => {
      ...
      const credentials = { mobile: '5553332222' };
      const response = await request(app)
        .post('/api/user/signin')
        .send(credentials)
        .expect(400);
